...
**hpsim** is a powerful multi-client traffic generator inspired by hping3. It simulates many clients on a single Linux host, sending customizable network traffic with spoofed IP/MAC addresses from the same subnet. Perfect for network testing, load testing, and security research.
...
# Navigate to the hpsim directory
cd tools/hpsim
```
...
### Step 4: Install hpsim

You can install hpsim in multiple ways:
...
python3 -m hpsim --help
```

### Step 5: Grant Network Capabilities

hpsim needs raw socket access to craft and send custom packets. You have two options:

#### Option A: Run with sudo (Simple but less secure)

```bash
sudo hpsim --interface eth0 --clients 5 --dst 10.0.0.5 --dport 80
```
...
hpsim --interface eth0 --clients 5 --dst 10.0.0.5 --dport 80
```
...
hpsim --interface eth0 --clients 2 --dst 192.168.1.1 --dport 80 --count 5 --dry-run

# Send a few test packets (requires root/capabilities)
hpsim --interface eth0 --clients 2 --dst 192.168.1.1 --dport 80 --count 5
```

### For Developers

If you plan to modify or contribute to hpsim:
...
pytest tests/ --cov=hpsim --cov-report=html
...
```bash
hpsim --interface eth0 --clients 20 --dst 192.168.1.100 --dport 53 --proto udp --count 500 --interval 0.005
```
...
```bash
hpsim --interface eth0 --clients 50 --dst 8.8.8.8 --proto icmp --icmp-type 8 --icmp-code 0 --count 1000 --flood
```
...
```bash
hpsim --interface eth0 --clients 15 --dest-subnet 10.0.0.0/24 --rand-dest --rand-source --proto tcp --dport 80 --count 200
```
...
```bash
# Text payload
hpsim --interface eth0 --clients 5 --dst 10.0.0.5 --dport 8080 --proto tcp --payload "Hello Server" --count 10

# Hex payload
hpsim --interface eth0 --clients 5 --dst 10.0.0.5 --dport 8080 --proto udp --payload "deadbeef" --payload-hex --count 10
```
...
```bash
hpsim --interface eth0 --clients 10 --dst 10.0.0.5 --dport 443 --proto tcp --flags S --count 100 --pcap-out traffic.pcap
```
...
hpsim --interface eth0 --clients 100 --dst webserver.local --dport 80 \
...
hpsim --interface eth0 --clients 500 --dest-subnet 10.0.0.0/24 \
...
hpsim --interface eth0 --clients 50 --dst 192.168.1.1 \
...
```bash
# Test ICMP handling
hpsim --interface eth0 --clients 20 --dst target.local \
    --proto icmp --icmp-type 8 --icmp-code 0 \
    --count 1000 --interval 0.01

# Test UDP services
hpsim --interface eth0 --clients 30 --dst dns-server.local \
...
hpsim --interface eth0 --clients 10 --dst 224.0.0.1 \
...
```bash
# Test GRE tunneling
hpsim --interface eth0 --clients 5 --dst tunnel-endpoint.local \
    --proto gre --payload "encapsulated" --count 200

# Test IPsec ESP
hpsim --interface eth0 --clients 5 --dst vpn-gateway.local \
    --proto esp --count 100
```
...
hpsim --interface eth0 --clients 10 --dst 10.0.0.5 --dport 80 \
...
hpsim --interface eth0 --clients 5 --dst 10.0.0.5 --dport 22 \
...
hpsim --interface eth0 --clients 100 --dst 10.0.0.5 --dport 80 \
...
hpsim --interface eth0 --clients 200 --dst web.example.com --dport 80 \
...
hpsim --interface eth0 --clients 50 --dst 8.8.8.8 --dport 53 \
...
hpsim --interface eth0 --clients 10 --dst firewall.local \
...
hpsim --interface eth0 --clients 100 --dest-subnet 192.168.1.0/24 \
    --rand-dest --rand-source --proto tcp --dport 80 --flags S \
...
hpsim supports a wide range of network protocols for comprehensive testing:
...
```bash
hpsim --interface eth0 --dst target --dport 80 --proto tcp --flags S
```
...
```bash
hpsim --interface eth0 --dst target --dport 53 --proto udp
```
...
```bash
hpsim --interface eth0 --dst target --dport 5060 --proto sctp
```
...
```bash
hpsim --interface eth0 --dst target --proto icmp --icmp-type 8 --icmp-code 0
```
...
```bash
hpsim --interface eth0 --dst 224.0.0.1 --proto igmp
```
...
```bash
hpsim --interface eth0 --dst tunnel-endpoint --proto gre
```
...
```bash
hpsim --interface eth0 --dst vpn-gateway --proto esp
```
...
```bash
hpsim --interface eth0 --dst vpn-gateway --proto ah
```
...
**Problem:** hpsim requires raw socket access to craft custom packets.

**Solutions:**
```bash
# Solution A: Run with sudo
sudo hpsim --interface eth0 --dst 10.0.0.5 --dport 80
...
hpsim --interface enp0s3 --dst 10.0.0.5 --dport 80
```
...
```bash
# Remove --dry-run flag
hpsim --interface eth0 --dst 10.0.0.5 --dport 80 --count 10
# Not: ... --dry-run
```
...
hpsim --interface eth0 --clients 50 --dst target --dport 80
# Instead of: --clients 10000

# Add interval (remove --flood)
hpsim --interface eth0 --clients 100 --dst target --dport 80 --interval 0.001
...
hpsim --interface eth0 --dst target --dport 80 --count 10 --pcap-out test.pcap
# Verify with Wireshark: wireshark test.pcap

# Enable verbose mode
hpsim --interface eth0 --dst target --dport 80 --count 10 --verbose
```

#### 7. Python version incompatibility

**Problem:** hpsim requires Python 3.8+.
...
hpsim --interface eth0 --dst 127.0.0.1 --dport 8080 --count 5 --dry-run

# Run with verbose output for debugging
hpsim --interface eth0 --dst target --dport 80 --count 10 --verbose

# Check packet crafting without sending
hpsim --interface eth0 --clients 5 --dst target --dport 80 --count 2 --dry-run --verbose
```
...
hpsim --interface eth0 --dst 192.168.1.1 --dport 80 --count 1 --dry-run --verbose

# Step 3: Send to localhost first
hpsim --interface lo --dst 127.0.0.1 --dport 8080 --count 5

# Step 4: Capture and inspect packets
hpsim --interface eth0 --dst target --dport 80 --count 10 --pcap-out debug.pcap
wireshark debug.pcap

# Step 5: Gradually increase complexity
hpsim --interface eth0 --clients 2 --dst target --dport 80 --count 10
hpsim --interface eth0 --clients 10 --dst target --dport 80 --count 100
```
...
```
hpsim/
├── hpsim/               # Main package directory
│   ├── __init__.py      # Package initialization
│   ├── __main__.py      # Entry point for `python -m hpsim`
...
hpsim includes comprehensive unit tests for all modules.
...
pytest --cov=hpsim --cov-report=term-missing
...
This project is provided as-is for educational and testing purposes. Ensure you have proper authorization before using hpsim on any network.
